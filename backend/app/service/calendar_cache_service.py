...
class CalendarCacheService:
    ...
    def __init__(self, redis_manager: AsyncRedisManager):
        ...
        self.ttl_seconds = settings.CACHE_TTL_SECONDS  # 6 минут
    ...
    async def _cache_individual_events(self, client, user_id: str, events: List[Dict[str, Any]]):
        """
        Кеширует отдельные события для быстрого доступа.
        """
        try:
            for event in events:
                if "id" in event:
                    event_key = f"{self.PREFIX_EVENT}{user_id}:{event['id']}"
    ...
    async def get_event(self, user_id: str, event_id: str) -> Optional[Dict[str, Any]]:
        """
        Получает конкретное событие из кеша.
        """
        try:
            async with self.redis_manager.get_client() as client:
            ...
            print(f"Error getting event from cache: {str(e)}")
            return None

    async def cache_sync_token(self, user_id: str, sync_token: str) -> bool:
        """
        Кеширует sync token для инкрементальной синхронизации.
        """
        try:
            async with self.redis_manager.get_client() as client:
    ...
    async def get_single_event(self, user_id: str, event_id: str) -> Optional[Dict[str, Any]]:
        """
        Получает одно событие из кеша (алиас для get_event).
        """
        return await self.get_event(user_id, event_id)

    async def cache_single_event(self, user_id: str, event_data: Dict[str, Any]) -> bool:
        """
        Кеширует одно событие (алиас для update_event_in_cache).
        """
        return await self.update_event_in_cache(user_id, event_data)
